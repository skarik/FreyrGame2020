...
var __this__=array_create(4);
__this__[@0]=gml_std_array_hx_iterator_hasNext;
__this__[@1]=gml_std_array_hx_iterator_next;
if (live_enabled) {
	__this__[@2]=argument[0];
	__this__[@3]=0;
}
return __this__;

#define gml_std_array_hx_iterator_hasNext
var __this__=argument[0];
if (live_enabled) {
	return __this__[3]<array_length_1d(__this__[2]);
} else return false;

#define gml_std_array_hx_iterator_next
var __this__=argument[0];
if (live_enabled) {
	return gml_std_haxe_boot_wget(__this__[2],__this__[@3]++);
...
var __this__=[mt_gml_builder];
array_copy(__this__,1,mq_gml_builder,1,11);
var l_src=argument[0];
if (live_enabled) {
	__this__[@9]=undefined;
	__this__[@8]=[];
	__this__[@7]=[];
	__this__[@6]=[];
	__this__[@5]=[];
	__this__[@3]=0;
	__this__[@2]=l_src;
	if(l_src[6]!=undefined)__this__[@1]=l_src[6]; else __this__[@1]=gml_parser_run(l_src);
	if(__this__[1]!=undefined){
		__this__[@4]=array_length_1d(__this__[1]);
		gml_builder_build_loop(__this__,l_src[3]);
	} else {
		__this__[@9]=g_gml_parser_error_text;
		__this__[@10]=g_gml_parser_error_pos;
	}
}
return __this__;

#define gml_builder_error_at
var __this__=argument[0],l_pos=argument[2];
if (live_enabled) {
	__this__[@9]=gml_pos_to_string(l_pos)+": "+argument[1];
	__this__[@10]=l_pos;
	return true;
} else return false;

#define gml_builder_error
var __this__=argument[0],l_tk=argument[2];
if (live_enabled) {
	return gml_builder_error_at(__this__,argument[1],l_tk[1]);
} else return false;

#define gml_builder_expect
var __this__=argument[0],l_tk=argument[2];
if (live_enabled) {
	return gml_builder_error(__this__,"Expected "+argument[1]+", got "+g_gml_token_names[l_tk[0]],l_tk);
} else return false;

#define gml_builder_expect_node
var __this__=argument[0],l_node=argument[2];
if (live_enabled) {
	return gml_builder_error_at(__this__,"Expected "+argument[1]+", got "+g_gml_node_names[l_node[0]],l_node[1]);
} else return false;
...
if (live_enabled) {
	...
	ds_list_add(l_nodes,__this__[11]);
	...
	var l_proc=true;
	var l_i;
	while(l_proc&&__this__[3]<__this__[4]){
		if(gml_builder_build_expr(__this__,1)){
		...
		ds_list_add(l_nodes,__this__[11]);
		if(__this__[3]<__this__[4]){
			var l__g=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
			switch(l__g[0]){
				case 18:
					__this__[@3]+=1;
...
var __this__=argument[0],l_pos=argument[1],l_sqb=argument[2];
if (live_enabled) {
	var l_args1=[];
	var l_proc=true;
	var l_seenComma=true;
	var l_func=__this__[11];
	while(l_proc&&__this__[3]<__this__[4]){
		var l_tk=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
		switch(l_tk[0]){
			case 21:
				if(l_sqb){
					return gml_builder_error(__this__,"Unexpected `)`",l_tk);
				} else {
					l_proc=false;
					__this__[@3]+=1;
				}
			...
			case 4:
				if(l_seenComma){
					return gml_builder_error(__this__,"Unexpected comma",l_tk);
				} else {
					l_seenComma=true;
					__this__[@3]+=1;
				}
			...
			default:
				if(l_seenComma){
					l_seenComma=false;
					if(gml_builder_build_expr(__this__,0))return true;
					gml_std_array_hx_push(l_args1,__this__[11]);
				} else return gml_builder_expect(__this__,"a comma or closing token.",l_tk);
		}
	}
	if(l_proc)return gml_builder_error_at(__this__,"Unclosed list",l_pos);
	if(l_sqb)__this__[@11]=[4,l_pos,l_args1]; else __this__[@11]=[17,l_pos,l_func,l_args1];
	return false;
} else return false;

#define gml_builder_build_expr
var __this__=argument[0],l_flags=argument[1];
if (live_enabled) {
	if(__this__[3]>=__this__[4])return gml_builder_error_at(__this__,"Expected an expression",gml_std_haxe_boot_wget(__this__[2],7));
	var l_proc,l_sep,l_i,l_n,l_s,l_tk;
	var l_tk0=gml_std_haxe_boot_wget(__this__[1],__this__[@3]++);
	var l_node,l_node2,l_nodes;
	switch(l_tk0[0]){
		case 12:__this__[@11]=[0,l_tk0[1]];break;
		case 13:__this__[@11]=[1,l_tk0[1],l_tk0[2],l_tk0[3]];break;
		case 14:__this__[@11]=[2,l_tk0[1],l_tk0[2]];break;
		case 22:
			l_proc=true;
			l_sep=true;
			l_nodes=[];
			while(l_proc&&__this__[3]<__this__[4]){
				switch(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)){
					case 23:
						__this__[@3]+=1;
						l_proc=false;
						break;
					case 4:
						if(l_sep){
							return gml_builder_expect(__this__,"a value or closing bracket",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
						} else {
							__this__[@3]+=1;
							l_sep=true;
						}
					...
					default:
						if(gml_builder_build_expr(__this__,0))return true;
						gml_std_array_hx_push(l_nodes,__this__[11]);
						l_sep=false;
				}
			}
			__this__[@11]=[4,l_tk0[1],l_nodes];
			break;
		case 10:
			var l_s2=l_tk0[2];
			switch(l_s2){
				case "self":__this__[@11]=[8,l_tk0[1]];break;
				case "other":__this__[@11]=[9,l_tk0[1]];break;
				default:
					if(ds_map_exists(g_gml_asset_index,l_s2)){
						l_i=g_gml_asset_index[?l_s2];
						__this__[@11]=[1,l_tk0[1],l_i,undefined];
					} else __this__[@11]=[7,l_tk0[1],l_s2];
			}
			break;
		case 26:__this__[@11]=[14,l_tk0[1],l_tk0[2]];break;
		case 9:
			switch(l_tk0[2]){
				case 0:
					var l_d=l_tk0[1];
					if(__this__[3]>=__this__[4]){
						__this__[@11]=[10,l_d];
					} else if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==5){
						l_tk=gml_std_haxe_boot_wget(__this__[1],__this__[@3]++);
						if(__this__[3]>=__this__[4]){
							return gml_builder_expect(__this__,"a field name",l_tk);
						} else {
							var l__g2=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
							if(l__g2[0]==10){
								__this__[@3]+=1;
								__this__[@11]=[35,l__g2[1],l__g2[2]];
							} else return gml_builder_expect(__this__,"a field name",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
						}
					} else __this__[@11]=[10,l_d];
					break;
				case 21:
					if(__this__[3]>=__this__[4]){
						return gml_builder_error_at(__this__,"Expected an opening square bracket",gml_std_haxe_boot_wget(__this__[2],7));
					} else if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==22){
						__this__[@3]+=1;
					} else return gml_builder_error(__this__,"Expected an opening square bracket",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
					if(gml_builder_build_expr(__this__,0))return true;
					if(__this__[3]>=__this__[4]){
						return gml_builder_error_at(__this__,"Expected a closing square bracket",gml_std_haxe_boot_wget(__this__[2],7));
					} else if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==23){
						__this__[@3]+=1;
					} else return gml_builder_error(__this__,"Expected a closing square bracket",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
					__this__[@11]=[15,l_tk0[1],__this__[11]];
					break;
				case 22:__this__[@11]=[16,l_tk0[1]];break;
				default:if((l_flags&4)!=0)return gml_builder_expect(__this__,"a statement",l_tk0); else return gml_builder_expect(__this__,"a value",l_tk0);
			}
			break;
		case 15:
			if(gml_builder_build_expr(__this__,1))return true;
			var l__g5=__this__[11];
			if(l__g5[0]==1){
				var l_f1=l__g5[2];
				switch(l_tk0[2]){
				...
				__this__[@11]=[1,l_tk0[1],l_f1,undefined];
			} else __this__[@11]=[26,l_tk0[1],__this__[11],l_tk0[2]];
			break;
		case 17:
			if(gml_builder_build_expr(__this__,1))return true;
			__this__[@11]=[24,l_tk0[1],__this__[11],l_tk0[2]];
			break;
		case 18:
			switch(l_tk0[2]){
				case 16:if(gml_builder_build_expr(__this__,1))return true;break;
				case 17:
					var l_d10=l_tk0[1];
					if(gml_builder_build_expr(__this__,1))return true;
					var l__g6=__this__[11];
					if(l__g6[0]==1)__this__[@11]=[1,l_d10,-l__g6[2],undefined]; else __this__[@11]=[26,l_d10,__this__[11],0];
					break;
				default:if((l_flags&4)!=0)return gml_builder_expect(__this__,"a statement",l_tk0); else return gml_builder_expect(__this__,"a value",l_tk0);
			}
			break;
		case 20:
			if(gml_builder_build_expr(__this__,0))return true;
			if(__this__[3]>=__this__[4])return gml_builder_error(__this__,"Unclosed parenthesis",l_tk0);
			if(__this__[3]>=__this__[4]){
				return gml_builder_error_at(__this__,"Expected a closing parenthesis",gml_std_haxe_boot_wget(__this__[2],7));
			} else if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==21){
				__this__[@3]+=1;
			} else return gml_builder_error(__this__,"Expected a closing parenthesis",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
			break;
		case 24:
			var l_keys=[];
			l_nodes=[];
			l_proc=true;
			var l__g9=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
			var l_tmp;
			if(l__g9[0]==25)l_tmp=true; else l_tmp=false;
			if(l_tmp){
				__this__[@3]+=1;
			} else while(l_proc&&__this__[3]<__this__[4]){
				var l__g22=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
				switch(l__g22[0]){
					case 14:
						l_s=l__g22[2];
						...
						__this__[@3]+=1;
						if(__this__[3]>=__this__[4]){
							return gml_builder_error_at(__this__,"Expected a `:` in object declaration",gml_std_haxe_boot_wget(__this__[2],7));
						} else if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==6){
							__this__[@3]+=1;
						} else return gml_builder_error(__this__,"Expected a `:` in object declaration",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
						if(gml_builder_build_expr(__this__,0))return true;
						gml_std_array_hx_push(l_nodes,__this__[11]);
						switch(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)){
							case 4:
								__this__[@3]+=1;
								if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==25){
									__this__[@3]+=1;
									l_proc=false;
								}
								break;
							case 25:
								...
								l_proc=false;
								break;
							default:return gml_builder_expect(__this__,"a `,` or a `}` in object declaration",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
						}
						break;
					case 10:
						l_s=l__g22[2];
						...
						break;
					default:return gml_builder_expect(__this__,"a key-value pair or a `}` in object declaration",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
				}
			}
			__this__[@11]=[5,l_tk0[1],l_keys,l_nodes];
			break;
		case 28:
			if(gml_builder_build_expr(__this__,l_flags))return true;
			__this__[@11]=[110,l_tk0[1],l_tk0[2],__this__[11]];
			break;
		case 27:
			if(gml_builder_build_expr(__this__,l_flags))return true;
			__this__[@11]=[108,l_tk0[1],l_tk0[2],__this__[11]];
			break;
		case 29:
			if(gml_builder_build_expr(__this__,l_flags))return true;
			__this__[@11]=[112,l_tk0[1],l_tk0[2],__this__[11],l_tk0[3]];
			break;
		case 0:if((l_flags&4)!=0)return gml_builder_error(__this__,"Expected a statement, got a header (did you miss a closing bracket?)",l_tk0); else return gml_builder_error(__this__,"Expected an expression, got a header (did you miss a closing parenthesis?)",l_tk0);
		default:if((l_flags&4)!=0)return gml_builder_expect(__this__,"a statement",l_tk0); else return gml_builder_expect(__this__,"a value",l_tk0);
	}
	l_proc=true;
	while(l_proc&&__this__[3]<__this__[4]){
		...
		switch(l_tk[0]){
			case 17:
				if((l_flags&2)==0){
					__this__[@3]+=1;
					__this__[@11]=[25,l_tk[1],__this__[11],l_tk[2]];
					l_flags|=2;
				} else if(l_tk[2]){
					return gml_builder_error_at(__this__,"Unexpected `++`",l_tk[1]);
				} else return gml_builder_error_at(__this__,"Unexpected `--`",l_tk[1]);
				break;
			case 5:
				if((l_flags&2)==0){
					__this__[@3]+=1;
					var l__g311=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
					if(l__g311[0]==10){
						l_s=l__g311[2];
						__this__[@3]+=1;
						__this__[@11]=[38,l_tk[1],__this__[11],l_s];
					} else return gml_builder_error(__this__,"Expected a field name",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
				} else return gml_builder_error(__this__,"Unexpected period",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
				break;
			case 20:
				if((l_flags&2)==0){
					__this__[@3]+=1;
					if(gml_builder_build_args(__this__,l_tk[1],false))return true;
				} else return gml_builder_error_at(__this__,"Unexpected `(`",l_tk[1]);
				break;
			case 22:
				var l_d24=l_tk[1];
				if((l_flags&2)==0){
					__this__[@3]+=1;
					if(__this__[3]>=__this__[4])return gml_builder_error_at(__this__,"Expected an index",gml_std_haxe_boot_wget(__this__[2],7));
					l_node=__this__[11];
					var l__g314=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
					switch(l__g314[0]){
						case 18:
							if(l__g314[2]==48){
								__this__[@3]+=1;
								if(gml_builder_build_expr(__this__,0))return true;
								if(__this__[3]>=__this__[4]){
									return gml_builder_error_at(__this__,"Expected a closing bracket",gml_std_haxe_boot_wget(__this__[2],7));
								} else if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==23){
									__this__[@3]+=1;
								} else return gml_builder_error(__this__,"Expected a closing bracket",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
								__this__[@11]=[73,l_d24,l_node,__this__[11]];
							} else l_proc=false;
							break;
						case 7:
							__this__[@3]+=1;
							...
							__this__[@11]=[78,l_d24,l_node,__this__[11]];
							break;
						case 2:
							__this__[@3]+=1;
							if(gml_builder_build_expr(__this__,0))return true;
							l_node2=__this__[11];
							if(__this__[3]>=__this__[4]){
								return gml_builder_error_at(__this__,"Expected a comma",gml_std_haxe_boot_wget(__this__[2],7));
							} else if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==4){
								__this__[@3]+=1;
							} else return gml_builder_error(__this__,"Expected a comma",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
							if(gml_builder_build_expr(__this__,0))return true;
							if(__this__[3]>=__this__[4]){
								return gml_builder_error_at(__this__,"Expected a closing bracket",gml_std_haxe_boot_wget(__this__[2],7));
							} else if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==23){
							...
							__this__[@11]=[83,l_d24,l_node,l_node2,__this__[11]];
							break;
						case 8:
							__this__[@3]+=1;
							if(gml_builder_build_expr(__this__,0))return true;
							if(__this__[3]>=__this__[4])return gml_builder_error_at(__this__,"Expected comma or a closing bracket",gml_std_haxe_boot_wget(__this__[2],7));
							switch(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)){
								case 4:
									__this__[@3]+=1;
									l_node2=__this__[11];
									...
									__this__[@11]=[68,l_d24,l_node,l_node2,__this__[11]];
									break;
								case 23:
									__this__[@3]+=1;
									__this__[@11]=[63,l_d24,l_node,__this__[11]];
									break;
								default:gml_builder_expect(__this__,"comma or a closing bracket",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
							}
							break;
						default:l_proc=false;
					}
					if(!l_proc){
						...
						switch(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)){
							case 4:
								...
								__this__[@11]=[58,l_d24,l_node,l_node2,__this__[11]];
								break;
							case 23:
								__this__[@3]+=1;
								__this__[@11]=[53,l_d24,l_node,__this__[11]];
								break;
							default:gml_builder_expect(__this__,"comma or a closing bracket",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
						}
					}
				} else return gml_builder_error_at(__this__,"Unexpected `[`",l_d24);
				break;
			case 15:
				if(l_tk[2]==1){
					var l_d20=l_tk[1];
					if((l_flags&2)==0){
						__this__[@3]+=1;
						if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==16){
							l_node=__this__[11];
							__this__[@3]+=1;
							if(gml_builder_build_expr(__this__,1))return true;
							__this__[@11]=[31,l_d20,l_node,__this__[11],true];
						} else __this__[@3]-=1;
					}
				} else l_proc=false;
				break;
			case 16:
				if((l_flags&2)==0){
					...
					__this__[@3]+=1;
					if(gml_builder_build_expr(__this__,1))return true;
					__this__[@11]=[31,l_tk[1],l_node,__this__[11],false];
				}
				break;
			case 18:
				if((l_flags&1)==0){
					if(gml_builder_build_ops(__this__,l_tk[1],l_tk[2]))return true;
					l_flags|=2;
				} else l_proc=false;
				break;
			case 19:
				if(l_tk[2]==-1){
					var l_p1=l_tk[1];
					if((l_flags&1)==0){
						if(gml_builder_build_ops(__this__,l_p1,64))return true;
						l_flags|=2;
					} else l_proc=false;
				} else l_proc=false;
				break;
			case 7:
				if((l_flags&1)==0){
					...
					l_node2=__this__[11];
					if(__this__[3]>=__this__[4]){
						return gml_builder_error_at(__this__,"Expected an else-colon",gml_std_haxe_boot_wget(__this__[2],7));
					} else if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==6){
						__this__[@3]+=1;
					} else return gml_builder_error(__this__,"Expected an else-colon",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
					if(gml_builder_build_expr(__this__,0))return true;
					__this__[@11]=[91,l_tk[1],l_node,l_node2,__this__[11]];
				} else l_proc=false;
				break;
			case 27:
				__this__[@3]+=1;
				__this__[@11]=[109,l_tk[1],__this__[11],l_tk[2]];
				break;
			case 28:__this__[@3]+=1;break;
			case 29:
				__this__[@3]+=1;
				__this__[@11]=[113,l_tk[1],__this__[11],l_tk[2],l_tk[3]];
				break;
			default:l_proc=false;
		}
	}
...
var __this__=argument[0];
if (live_enabled) {
	if(__this__[3]>=__this__[4])return gml_builder_error_at(__this__,"Expected a statement",gml_std_haxe_boot_wget(__this__[2],7));
	var l_tk=gml_std_haxe_boot_wget(__this__[1],__this__[@3]++);
	...
	switch(l_tk[0]){
		case 9:
			...
			switch(l_tk[2]){
				case 1:
					l_d=l__g3;
					__this__[@11]=[89,l_d,[]];
					while(__this__[3]<__this__[4]){
						l_tk2=gml_std_haxe_boot_wget(__this__[1],__this__[@3]++);
						if(l_tk2[0]==10){
							l_d=l_tk2[1];
							l_s=l_tk2[2];
							l_i=array_length_1d(__this__[7]);
							gml_std_haxe_boot_wset(__this__[7],l_i,l_s);
							gml_std_haxe_boot_wset(__this__[8],l_i,[35,l_d,l_s]);
							if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==4){
								__this__[@3]+=1;
								continue;
							}
						} else return gml_builder_error(__this__,"Expected a global variable name.",l_tk2);
						break;
					}
					break;
				case 2:
					...
					while(l_proc&&__this__[3]<__this__[4]){
						l_tk2=gml_std_haxe_boot_wget(__this__[1],__this__[@3]++);
						if(l_tk2[0]==10){
							if(__this__[3]>=__this__[4])return gml_builder_error_at(__this__,"Expected a variable value",gml_std_haxe_boot_wget(__this__[2],7));
							l_tk=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
							if(l_tk[0]==19){
								if(l_tk[2]==-1){
									__this__[@3]+=1;
									if(gml_builder_build_expr(__this__,0))return true;
								} else __this__[@11]=undefined;
							} else __this__[@11]=undefined;
							gml_std_array_hx_push(l_nodes,[88,l_tk2[1],l_tk2[2],__this__[11]]);
							if(__this__[3]<__this__[4])switch(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)){
								case 4:__this__[@3]+=1;break;
								case 3:
									__this__[@3]+=1;
									l_proc=false;
									break;
						...
						} else return gml_builder_expect(__this__,"a variable name",l_tk2);
					}
					if(array_length_1d(l_nodes)!=1)__this__[@11]=[89,l_d2,l_nodes]; else __this__[@11]=l_nodes[0];
					break;
				case 3:
					var l_e;
					var l__g5=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
					if(l__g5[0]==10){
						var l_s2=l__g5[2];
						var l_d5=l__g5[1];
						__this__[@3]+=1;
						l_e=gml_enum_create(l_s2,l_d5);
					} else return gml_builder_error(__this__,"Expected an enum name",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
					if(__this__[3]>=__this__[4]){
						return gml_builder_error_at(__this__,"Expected enum block",gml_std_haxe_boot_wget(__this__[2],7));
					} else if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==24){
						__this__[@3]+=1;
					} else return gml_builder_error(__this__,"Expected enum block",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
					l_proc=true;
					l_sep=true;
					while(__this__[3]<__this__[4]&&l_proc){
						var l__g33=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
						switch(l__g33[0]){
							case 25:
								__this__[@3]+=1;
								l_proc=false;
							...
							case 4:
								if(l_sep){
									return gml_builder_error(__this__,"Unexpected comma",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
								} else {
									__this__[@3]+=1;
									l_sep=true;
								}
							...
							case 10:
								if(l_sep){
									__this__[@3]+=1;
									var l__g34=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
									if(l__g34[0]==19){
										if(l__g34[2]==-1){
											__this__[@3]+=1;
											if(gml_builder_build_expr(__this__,0))return true;
									...
									var l_ec=gml_enum_ctr_create(l__g33[2],l__g33[1],__this__[11]);
									gml_std_array_hx_push(l_e[3],l_ec);
									ds_map_set(l_e[4],l_ec[1],l_ec);
									l_sep=false;
								} else return gml_builder_expect(__this__,"a comma or a closing bracket",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
								break;
							default:return gml_builder_expect(__this__,"a comma, enum entry, or closing bracket",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
						}
					}
					if(l_proc)return gml_builder_error(__this__,"Unclosed enum-block",l_tk);
					gml_std_array_hx_push(__this__[6],l_e);
					__this__[@11]=[89,l__g3,[]];
					break;
				case 4:
					if(gml_builder_build_expr(__this__,0))return true;
					l_x1=__this__[11];
					if(__this__[3]>=__this__[4])return gml_builder_error_at(__this__,"Expected a then-expression",gml_std_haxe_boot_wget(__this__[2],7));
					var l__g8=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
					if(l__g8[0]==9){
						if(l__g8[2]==5)__this__[@3]+=1;
					}
					if(gml_builder_build_line(__this__))return true;
					l_x2=__this__[11];
					l_i=__this__[3];
					while(__this__[3]<__this__[4]){
						switch(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)){
					...
					if(__this__[3]<__this__[4]){
						var l__g36=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
						if(l__g36[0]==9){
							if(l__g36[2]==6){
								__this__[@3]+=1;
								if(gml_builder_build_line(__this__))return true;
					...
					__this__[@11]=[90,l__g3,l_x1,l_x2,l_x];
					break;
				case 7:
					var l_d7=l__g3;
					if(gml_builder_build_expr(__this__,0))return true;
					...
					var l_c=undefined;
					if(__this__[3]>=__this__[4]){
						return gml_builder_error_at(__this__,"Expected switch-block body",gml_std_haxe_boot_wget(__this__[2],7));
					} else if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==24){
						__this__[@3]+=1;
					} else return gml_builder_error(__this__,"Expected switch-block body",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
					if(__this__[3]>=__this__[4])return gml_builder_error_at(__this__,"Expected switch-block contents",gml_std_haxe_boot_wget(__this__[2],7));
					var l__g15=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
					switch(l__g15[0]){
						case 9:
							switch(l__g15[2]){
								case 8:case 9:break;
								default:return gml_builder_expect(__this__,"`case` or `default`",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
							}
							break;
						case 29:break;
						case 28:break;
					...
					var l_pre=[];
					while(__this__[3]<__this__[4]&&l_proc){
						var l__g101=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
						switch(l__g101[0]){
							case 25:
								__this__[@3]+=1;
								l_proc=false;
							...
							case 9:
								switch(l__g101[2]){
									case 8:
										...
										if(gml_builder_build_expr(__this__,0))return true;
										if(__this__[3]>=__this__[4]){
											return gml_builder_error_at(__this__,"Expected a colon",gml_std_haxe_boot_wget(__this__[2],7));
										} else if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==6){
											__this__[@3]+=1;
										} else return gml_builder_error(__this__,"Expected a colon",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
										l_nodes=[__this__[11]];
										while(__this__[3]<__this__[4]){
											var l__g103=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
											if(l__g103[0]==9){
												if(l__g103[2]==8){
													__this__[@3]+=1;
													if(gml_builder_build_expr(__this__,0))return true;
					...
					if(l_proc)return gml_builder_error_at(__this__,"Unclosed switch-block",l_d7);
					__this__[@11]=[92,l_d7,l_x1,l_cc,l_x2];
					break;
				case 14:
					if(__this__[3]>=__this__[4])return gml_builder_error_at(__this__,"Expected for-loop header",gml_std_haxe_boot_wget(__this__[2],7));
					if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==20){
						__this__[@3]+=1;
						l_proc=true;
					} else l_proc=false;
					if(gml_builder_build_line(__this__))return true;
					...
					if(gml_builder_build_expr(__this__,0))return true;
					l_x1=__this__[11];
					if(__this__[3]>=__this__[4])return gml_builder_error_at(__this__,"Expected for-loop post-action",gml_std_haxe_boot_wget(__this__[2],7));
					if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==3)__this__[@3]+=1;
					if(gml_builder_build_line(__this__))return true;
					l_x2=__this__[11];
					if(l_proc){
						if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==21)__this__[@3]+=1; else return gml_builder_expect(__this__,"a closing parenthesis",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
					}
					if(gml_builder_build_line(__this__))return true;
					__this__[@11]=[99,l__g3,l_x,l_x1,l_x2,__this__[11]];
					break;
				case 11:
					if(gml_builder_build_expr(__this__,0))return true;
					l_x1=__this__[11];
					if(gml_builder_build_line(__this__))return true;
					__this__[@11]=[95,l__g3,l_x1,__this__[11]];
					break;
				case 10:
					if(gml_builder_build_expr(__this__,0))return true;
					l_x1=__this__[11];
					if(gml_builder_build_line(__this__))return true;
					__this__[@11]=[98,l__g3,l_x1,__this__[11]];
					break;
				case 13:
					var l_d13=l__g3;
					if(gml_builder_build_line(__this__))return true;
					l_x1=__this__[11];
					if(__this__[3]>=__this__[4])return gml_builder_error_at(__this__,"Expected a `while` or `until`",gml_std_haxe_boot_wget(__this__[2],7));
					var l__g17=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
					if(l__g17[0]==9)switch(l__g17[2]){
						case 11:
							__this__[@3]+=1;
							if(gml_builder_build_expr(__this__,0))return true;
							__this__[@11]=[97,l_d13,l_x1,__this__[11]];
							break;
						case 12:
							__this__[@3]+=1;
							if(gml_builder_build_expr(__this__,0))return true;
							__this__[@11]=[96,l_d13,l_x1,__this__[11]];
							break;
						default:return gml_builder_expect(__this__,"a `while` or `until`",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
					} else return gml_builder_expect(__this__,"a `while` or `until`",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
					break;
				case 15:
					if(gml_builder_build_expr(__this__,0))return true;
					l_x1=__this__[11];
					if(gml_builder_build_line(__this__))return true;
					__this__[@11]=[100,l__g3,l_x1,__this__[11]];
					break;
				case 17:__this__[@11]=[104,l__g3];break;
				case 16:__this__[@11]=[105,l__g3];break;
				case 19:__this__[@11]=[103,l__g3];break;
				case 18:
					var l_d18=l__g3;
					if(__this__[3]<__this__[4]){
						var l__g24=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
						switch(l__g24[0]){
							case 25:__this__[@11]=[103,l__g24[1]];break;
							case 3:__this__[@11]=[103,l__g24[1]];break;
							default:
								if(gml_builder_build_expr(__this__,0))return true;
								__this__[@11]=[102,l_d18,__this__[11]];
						}
					} else __this__[@11]=[103,l_d18];
					break;
				case 20:
					if(gml_builder_build_expr(__this__,0))return true;
					__this__[@11]=[93,l__g3,__this__[11]];
					break;
				case 23:__this__[@11]=[106,l__g3];break;
		...
		case 1:
			...
			if(l__g[0]==10){
				if(gml_builder_build_expr(__this__,0))return true;
				var l_i1=array_length_1d(__this__[7]);
				gml_std_haxe_boot_wset(__this__[7],l_i1,l__g[2]);
				gml_std_haxe_boot_wset(__this__[8],l_i1,__this__[11]);
				__this__[@11]=[89,l_tk[1],[]];
			} else gml_builder_error(__this__,"Expected a macro name",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
			break;
		case 24:
			var l_start=l_tk[1];
			l_proc=true;
			l_nodes=[];
			if(__this__[3]<__this__[4]){
				var l__g25=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
				if(l__g25[0]==27){
			...
			while(l_proc&&__this__[3]<__this__[4]){
				var l__g26=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
				switch(l__g26[0]){
					case 25:
						__this__[@3]+=1;
						l_proc=false;
					...
					case 29:
						if(l__g26[3]){
							...
							if(gml_builder_build_line(__this__))return true;
							gml_std_array_hx_push(l_nodes,[112,l__g26[1],l__g26[2],__this__[11],false]);
						}
						break;
					default:
						if(gml_builder_build_line(__this__))return true;
			...
			if(l_proc)return gml_builder_error_at(__this__,"Expected a closing bracket.",l_start);
			__this__[@11]=[89,l_start,l_nodes];
			break;
		case 28:
			if(__this__[3]<__this__[4]){
				if(gml_builder_build_line(__this__))return true;
				__this__[@11]=[110,l_tk[1],l_tk[2],__this__[11]];
			} else __this__[@11]=[107,l_tk[1],l_tk[2]];
			break;
		case 27:
			if(gml_builder_build_line(__this__))return true;
			__this__[@11]=[108,l_tk[1],l_tk[2],__this__[11]];
			break;
		case 29:
			if(__this__[3]<__this__[4]){
				if(gml_builder_build_line(__this__))return true;
				__this__[@11]=[112,l_tk[1],l_tk[2],__this__[11],l_tk[3]];
			} else __this__[@11]=[111,l_tk[1],l_tk[2]];
			break;
		default:l_unknown=true;
	}
	if(l_unknown)__this__[@3]-=1;
	if(l_unknown){
		if(gml_builder_build_expr(__this__,5))return true;
		if(__this__[3]<__this__[4]){
			var l__g115=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
			if(l__g115[0]==19){
				l_x=__this__[11];
				__this__[@3]+=1;
				if(gml_builder_build_expr(__this__,0))return true;
				__this__[@11]=[28,l__g115[1],l__g115[2],l_x,__this__[11]];
			} else if(!gml_node_is_statement(__this__[11])){
				return gml_builder_expect_node(__this__,"a statement",__this__[11]);
			}
		} else if(!gml_node_is_statement(__this__[11])){
			return gml_builder_expect_node(__this__,"a statement",__this__[11]);
		}
	...
	while(__this__[3]<__this__[4]){
		if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==3){
			__this__[@3]+=1;
			continue;
		}
		break;
	...
	if(__this__[3]<__this__[4]){
		var l__g117=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
		if(l__g117[0]==27){
			__this__[@3]+=1;
			__this__[@11]=[109,l__g117[1],__this__[11],l__g117[2]];
		}
	}
	return false;
} else return false;
...
var __this__=argument[0],l_namedArgs,l_namedArgc;
...
if (live_enabled) {
	var l_scr=gml_script_create(__this__[2],argument[1],(__this__[3]>=__this__[4]?gml_std_haxe_boot_wget(__this__[2],7):gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),1)));
	if(l_namedArgs==undefined)l_namedArgs=ds_map_create();
	l_scr[@9]=l_namedArgs;
	l_scr[@8]=l_namedArgc;
	gml_std_array_hx_push(__this__[5],l_scr);
	var l_nodes=[];
	while(__this__[3]<__this__[4]){
		var l__g=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
		switch(l__g[0]){
	...
	if(array_length_1d(l_nodes)>1){
		...
		l_scr[@4]=l_nodes[0];
	} else l_scr[@4]=[89,gml_std_haxe_boot_wget(__this__[2],7),l_nodes];
	return false;
} else return false;
...
var __this__=argument[0];
if (live_enabled) {
	if(gml_builder_build_outer(__this__,argument[1]))return true;
	while(__this__[3]<__this__[4]){
		var l_tk=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
		if(l_tk[0]==0){
			__this__[@3]+=1;
			...
			var l_nextArgc=0;
			if(!(l_tk[3]||__this__[3]>=__this__[4])){
				if(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)==20){
					__this__[@3]+=1;
					l_nextArgs=ds_map_create();
					var l_proc=true;
					if(__this__[3]>=__this__[4])return gml_builder_error_at(__this__,"Expected script arguments",gml_std_haxe_boot_wget(__this__[2],7));
					var l__g2=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
					var l_tmp;
					...
					if(l_tmp){
						...
						var l__g21=gml_std_haxe_boot_wget(__this__[1],__this__[3]);
						if(l__g21[0]==10){
							var l_s=l__g21[2];
							var l_nextArg=l_s;
							if(ds_map_exists(l_nextArgs,l_nextArg)){
								ds_map_destroy(l_nextArgs);
								return gml_builder_error(__this__,"An argument named "+l_nextArg+" is already defined at position "+gml_std_Std_string(l_nextArgs[?l_nextArg]),gml_std_haxe_boot_wget(__this__[1],__this__[3]));
							}
							...
							switch(gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[1],__this__[3]),0)){
								...
								default:
									ds_map_destroy(l_nextArgs);
									return gml_builder_expect(__this__,"a comma or closing parenthesis in script arguments",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
							}
						} else return gml_builder_expect(__this__,"an argument",gml_std_haxe_boot_wget(__this__[1],__this__[3]));
					}
				}
			}
			if(gml_builder_build_outer(__this__,l_nextName,l_nextArgs,l_nextArgc))return true;
		} else return gml_builder_expect(__this__,"A script declaration",l_tk);
	}
	return false;
} else return false;
...
var __this__=[mt_gml_program];
array_copy(__this__,1,mq_gml_program,1,15);
var l_sources=argument[0];
if (live_enabled) {
	__this__[@13]=false;
	__this__[@11]=undefined;
	__this__[@10]=undefined;
	__this__[@9]=undefined;
	__this__[@8]=ds_list_create();
	__this__[@7]=ds_list_create();
	__this__[@6]=ds_map_create();
	__this__[@5]=ds_map_create();
	__this__[@4]=[];
	__this__[@3]=ds_map_create();
	__this__[@2]=[];
	__this__[@1]=undefined;
	__this__[@1]=l_sources;
	var l_builders=[];
	var l__g=0;
	while(l__g<array_length_1d(l_sources)){
		...
		if(l_b[9]==undefined){
			...
			while(l__g1<array_length_1d(l__g11)){
				...
				++l__g1;
				if(ds_map_exists(__this__[3],l_scr[1])){
					if(l_scr[1]==l_main){
						var l__g2=gml_std_haxe_boot_wget(ds_map_find_value(__this__[3],l_main),4);
						var l_tmp;
						...
						if(l_tmp){
							...
							ds_map_set(__this__[3],l_scr[1],l_scr);
						} else {
							gml_program_error(__this__,"Cannot override prefix-script \""+l_main+"\" because it is not empty",l_scr[2]);
							return __this__;
						}
					} else {
						gml_program_error(__this__,"Script "+l_scr[1]+" is already defined at "+gml_pos_to_string(gml_std_haxe_boot_wget(ds_map_find_value(__this__[3],l_scr[1]),2)),l_scr[2]);
						return __this__;
					}
				} else {
					gml_std_array_hx_push(__this__[2],l_scr);
					ds_map_set(__this__[3],l_scr[1],l_scr);
				}
			}
			var l__g21=0;
			...
			while(l__g21<array_length_1d(l__g3)){
				...
				++l__g21;
				gml_std_array_hx_push(__this__[4],l_e);
				ds_map_set(__this__[5],l_e[1],l_e);
			}
			var l_mcrNames=l_b[7];
			var l_mcrNodes=l_b[8];
			var l_mcrMap=__this__[6];
			var l_i1=0;
			...
			if(__this__[11]!=undefined)__this__[@11]+="\n"+l_errorNext; else __this__[@11]=l_errorNext;
		} else {
			__this__[@11]=l_b[9];
			__this__[@12]=l_b[10];
			return __this__;
	...
	var l_i2;
	var l_n1=array_length_1d(__this__[2]);
	for(l_i2=0;l_i2<l_n1;++l_i2){
		gml_std_haxe_boot_wset(gml_std_haxe_boot_wget(__this__[2],l_i2),3,g_gml_script_index_offset+l_i2);
	}
	if(gml_program_check(__this__)){
		g_gml_program_seek_inst=undefined;
		return __this__;
	} else if(gml_compile_program(__this__)){
		if(__this__[11]!=undefined)__this__[@11]+="\n"+g_gml_compile_error_text; else __this__[@11]=g_gml_compile_error_text;
		__this__[@12]=g_gml_compile_error_pos;
		return __this__;
	}
	__this__[@13]=true;
}
...
if (live_enabled) {
	...
	__this__[@11]=gml_pos_to_string(l_pos)+" "+argument[1];
	__this__[@12]=l_pos;
	return true;
} else return false;
...
if (live_enabled) {
	var l__g=0;
	var l__g1=__this__[2];
	while(l__g<array_length_1d(l__g1)){
		var l_q=l__g1[l__g];
		++l__g;
		gml_script_destroy(l_q);
	}
	__this__[@2]=undefined;
	ds_map_destroy(__this__[3]);
	__this__[@3]=undefined;
	ds_map_destroy(__this__[5]);
	__this__[@5]=undefined;
	ds_map_destroy(__this__[6]);
	__this__[@6]=undefined;
	ds_list_destroy(__this__[7]);
	__this__[@7]=undefined;
	ds_list_destroy(__this__[8]);
	__this__[@8]=undefined;
}

#define gml_program_call_v
var __this__=argument[0],l_name=argument[1],l_args1=argument[2],l_copy;
if(argument_count>3)l_copy=argument[3];else l_copy=true;
if (live_enabled) {
	if(ds_map_exists(__this__[3],l_name)){
		var l_scr=ds_map_find_value(__this__[3],l_name);
		var l_locals=array_create(l_scr[7]);
		if(l_copy)l_args1=gml_value_list_copy(l_args1);
		var l_th=gml_thread_create(__this__,l_scr[10],l_args1,l_locals);
		l_th[@3]=__this__[9];
		l_th[@6]=__this__[10];
...
var __this__=argument[0],l_name=argument[1];
if (live_enabled) {
	...
	if(ds_map_exists(__this__[3],l_name)){
		...
		var l_th=gml_thread_create(__this__,l_scr[10],l_args2,l_locals);
		l_th[@3]=__this__[9];
		l_th[@6]=__this__[10];
		gml_thread_exec(l_th);
		return l_th;
...
var __this__=argument[0],l_scriptId=argument[1],l_args1=argument[2],l_copy;
if(argument_count>3)l_copy=argument[3];else l_copy=true;
if (live_enabled) {
	l_scriptId-=g_gml_script_index_offset;
	var l_tmp;
	if(l_scriptId>=0)l_tmp=l_scriptId<array_length_1d(__this__[2]); else l_tmp=false;
	if(l_tmp){
		var l_scr=gml_std_haxe_boot_wget(__this__[2],l_scriptId);
		var l_locals=array_create(l_scr[7]);
		if(l_copy)l_args1=gml_value_list_copy(l_args1);
		var l_th=gml_thread_create(__this__,l_scr[10],l_args1,l_locals);
		l_th[@3]=__this__[9];
...
if (live_enabled) {
	...
	var l_tmp;
	if(l_scriptId1>=0)l_tmp=l_scriptId1<array_length_1d(__this__[2]); else l_tmp=false;
	if(l_tmp){
		var l_scr=gml_std_haxe_boot_wget(__this__[2],l_scriptId1);
		var l_locals=array_create(l_scr[7]);
		if(l_copy)l_args2=gml_value_list_copy(l_args2);
		var l_th=gml_thread_create(__this__,l_scr[10],l_args2,l_locals);
		l_th[@3]=__this__[9];
...
if (live_enabled) {
	var l_r="";
	var l__g=0;
	var l__g1=array_length_1d(__this__[2]);
	while(l__g<l__g1){
		var l_i=l__g++;
		if(l_i>0)l_r+="\n";
		var l_scr=gml_std_haxe_boot_wget(__this__[2],l_i);
...
var __this__=argument[0],l_f=argument[1],l_st;
...
var __this__=argument[0];
if (live_enabled) {
	g_gml_program_seek_inst=__this__;
	if(gml_program_seek(__this__,f_gml_seek_arguments_proc,false))return true;
	if(gml_program_seek(__this__,f_gml_seek_locals_proc,false))return true;
	if(gml_program_seek(__this__,f_gml_seek_idents_proc,true))return true;
	if(gml_program_seek(__this__,f_gml_seek_fields_proc,false))return true;
	if(gml_program_seek(__this__,f_gml_seek_calls_proc,false))return true;
	if(gml_seek_enum_values_proc())return true;
	if(gml_program_seek(__this__,f_gml_seek_enum_fields_proc,false))return true;
	if(gml_seek_eval_opt())return true;
	if(gml_program_seek(__this__,f_gml_seek_self_fields_proc,false))return true;
	if(gml_program_seek(__this__,f_gml_seek_alarms_proc,false))return true;
	if(gml_program_seek(__this__,f_gml_seek_adjfix_proc,true))return true;
	if(gml_program_seek(__this__,f_gml_seek_set_op_proc,true))return true;
	g_gml_program_seek_inst=undefined;
	return false;
} else return false;

#define gml_program_eval
var __this__=argument[0],l_q=argument[1];
if (live_enabled) {
	var l_r,l_v;
	var l__g=l_q;
	switch(l__g[0]){
		...
		case 38:
			...
			if(l__hx_tmp[0]==7){
				...
				var l_s=l__hx_tmp[2];
				var l_d1=l__g[1];
				var l_e=ds_map_find_value(__this__[5],l_s);
				if(l_e!=undefined){
					var l_c=ds_map_find_value(l_e[4],l_f);
					if(l_c!=undefined){
						l_r=l_c[4];
						if(l_r==undefined)return gml_program_error(__this__,"Value of "+l_s+"."+l_f+" is not known here",l_d1);
					} else return gml_program_error(__this__,"Enum `"+l_s+"` does not contain field `"+l_f+"`",l_d1);
				} else return gml_program_error(__this__,"Could not find enum "+l_s,l_d1);
			} else return gml_program_error(__this__,"Can not evaluate __this__ compile-time",l_q[1]);
			break;
		case 27:
			if(gml_program_eval(__this__,l__g[3]))return true;
			l_r=__this__[15];
			if(gml_program_eval(__this__,l__g[4]))return true;
			l_v=__this__[15];
			switch(l__g[2]){
				...
				default:return gml_program_error(__this__,"Can not evaluate __this__ compile-time",l_q[1]);
			}
			break;
		default:return gml_program_error(__this__,"Can not evaluate __this__ compile-time",l_q[1]);
	}
...
var __this__=argument[0],l_startPos;
if(argument_count>2)l_startPos=argument[2];else l_startPos=0;
if (live_enabled) {
	var l_out=string_pos(argument[1],(l_startPos>0?string_delete(__this__,1,l_startPos):__this__));
	if(l_out>0)return l_out+l_startPos-1; else return -1;
} else return 0;

#define gml_std_string_split
var __this__=argument[0],l_del=argument[1];
if (live_enabled) {
	var l_str=__this__;
	var l_num=0;
...
var __this__=argument[0],l_pos=argument[1],l_len;
if(argument_count>2)l_len=argument[2];else l_len=undefined;
if (live_enabled) {
	if(l_pos<0)l_pos+=string_length(__this__);
	if(l_len==undefined)return string_delete(__this__,1,l_pos); else return string_copy(__this__,1+l_pos,l_len);
} else return undefined;

#define gml_std_string_substring
var __this__=argument[0],l_start=argument[1],l_end;
if(argument_count>2)l_end=argument[2];else l_end=undefined;
if (live_enabled) {
	if(l_end==undefined){
		if(l_start>0)return string_delete(__this__,1,l_start); else return __this__;
	}
	...
	var l_len=string_length(__this__);
	if(l_start>=l_len){
		return "";
	} else if(l_end>=l_len){
		if(l_start>0)return string_delete(__this__,1,l_start); else return __this__;
	} else return string_copy(__this__,l_start+1,l_end-l_start);
} else return undefined;
...
var __this__=[mt_gml_enum];
array_copy(__this__,1,mq_gml_enum,1,4);
if (live_enabled) {
	__this__[@4]=ds_map_create();
	__this__[@3]=[];
	__this__[@1]=argument[0];
	__this__[@2]=argument[1];
}
return __this__;

#define gml_enum_add
var __this__=argument[0],l_name=argument[1],l_val=argument[2];
if (live_enabled) {
	var l_ctr=gml_enum_ctr_create(l_name,__this__[2],[1,__this__[2],l_val,string(l_val)]);
	l_ctr[@4]=l_val;
	gml_std_array_hx_push(__this__[3],l_ctr);
	ds_map_set(__this__[4],l_name,l_ctr);
}

#define gml_enum_print
var __this__=argument[0];
if (live_enabled) {
	var l_r="(enum "+__this__[1]+" { ";
	var l_z=false;
	var l__g=0;
	var l__g1=__this__[3];
	while(l__g<array_length_1d(l__g1)){
...
var __this__=[mt_gml_enum_ctr];
array_copy(__this__,1,mq_gml_enum_ctr,1,4);
if (live_enabled) {
	__this__[@1]=argument[0];
	__this__[@2]=argument[1];
	__this__[@3]=argument[2];
}
return __this__;
...
if (live_enabled) {
	switch(l___this__1){
		...
		case 80:return "&&";
		case 96:return "||";
		default:return gml_op_get_name(l___this__1);
	}
} else return undefined;
...
var __this__=[mt_gml_pos];
array_copy(__this__,1,mq_gml_pos,1,3);
if (live_enabled) {
	__this__[@1]=argument[0];
	__this__[@2]=argument[1];
	__this__[@3]=argument[2];
...
var __this__=argument[0];
if (live_enabled) {
	return gml_std_haxe_boot_wget(__this__[1],1)+("[L"+string(__this__[2])+",c"+string(__this__[3])+"]");
} else return undefined;

#define gml_script_create
var __this__=[mt_gml_script];
array_copy(__this__,1,mq_gml_script,1,10);
if (live_enabled) {
	__this__[@10]=undefined;
	__this__[@9]=undefined;
	__this__[@8]=0;
	__this__[@7]=0;
	__this__[@6]=ds_map_create();
	__this__[@5]=argument[0];
	__this__[@1]=argument[1];
	__this__[@2]=argument[2];
}
return __this__;
...
if (live_enabled) {
	ds_map_destroy(__this__[6]);
	if(__this__[10]!=undefined){
		var l___this__2=__this__[10];
		var l_i=0;
		for(var l__g1=ds_list_size(l___this__2);l_i<l__g1;l_i++){
			var l_q=l___this__2[|l_i];
			if(l_q[0]==58)ds_map_destroy(l_q[2]);
		}
		ds_list_destroy(l___this__2);
	}
}
...
if (live_enabled) {
	...
	g_gml_program_seek_script=__this__;
	script_execute(argument[1],__this__[4],argument[2]);
	g_gml_program_seek_script=l__scr;
}

#define gml_source_create
var __this__=[mt_gml_source];
array_copy(__this__,1,mq_gml_source,1,7);
...
if (live_enabled) {
	...
	__this__[@3]=l_main;
	__this__[@4]=string_length(l_code);
	__this__[@7]=gml_pos_create(__this__,string_count("\n",l_code)+2,1);
}
return __this__;
...
if (live_enabled) {
	...
	if(gml_value_is_number(l_seek)){
		...
		if(array_length_2d(l_seek,2)==1){
			...
			var l___this__4=l_seek;
			l_n=script_execute(l___this__4[1, 0],4,l___this__4);
...
var __this__=["mt_gml_std_haxe_class"];
array_copy(__this__,1,mq_gml_std_haxe_class,1,4);
if (live_enabled) {
	__this__[@4]=undefined;
	__this__[@1]=gml_std_haxe_type_markerValue;
	__this__[@2]=argument[0];
	__this__[@3]=argument[1];
}
return __this__;
...
if (live_enabled) {
	if(l_snippet[2]!=undefined)gml_program_destroy(l_snippet); else show_error("__this__ snippet is already destroyed",false);
}
...
var __this__=array_create(2);
if (live_enabled) {
	__this__[@0]=argument[0];
	__this__[@1]=argument[1];
}
return __this__;
...
var l___this__1=argument[0];
if (live_enabled) {
	var l_i=l___this__1[0]+1;
	if(l_i>=array_length_1d(l___this__1))l___this__1[@array_length_1d(l___this__1)*2]=0;
	l___this__1[@l_i]=argument[1];
	l___this__1[@0]=l_i;
}
...
if (live_enabled) {
	var l_i=l___this__1[0]+1;
	if(l_i>=array_length_1d(l___this__1))l___this__1[@array_length_1d(l___this__1)*2]=0;
	l___this__1[@l_i]=l___this__1[l_i-1];
	l___this__1[@0]=l_i;
}
...
var l___this__1=argument[0];
if (live_enabled) {
	var l_i=l___this__1[0]+2;
	if(l_i>=array_length_1d(l___this__1))l___this__1[@array_length_1d(l___this__1)*2]=0;
	l___this__1[@l_i-1]=l___this__1[l_i-3];
	l___this__1[@l_i]=l___this__1[l_i-2];
	l___this__1[@0]=l_i;
}
...
var l___this__1=argument[0];
if (live_enabled) {
	var l_i=l___this__1[0];
	var l_v=l___this__1[l_i++];
	l___this__1[@0]=l_i;
	var l_k=l_i-argument[1];
	if(l_i>=array_length_1d(l___this__1))l___this__1[@array_length_1d(l___this__1)*2]=0;
	while(--l_i>=l_k){
		l___this__1[@l_i+1]=l___this__1[l_i];
	}
	l___this__1[@l_k]=l_v;
}
...
var l___this__1=argument[0],l_n=argument[1];
if (live_enabled) {
	var l_i=l___this__1[0];
	l_i-=l_n;
	array_copy(l___this__1,l_i,g_tools__gml_stack_gml_stack_impl__fill_value_arr,0,l_n);
	l___this__1[@0]=l_i;
}
...
if (live_enabled) {
	var l_r="";
	var l_i=0;
	for(var l__g1=ds_list_size(l___this__1);l_i<l__g1;l_i++){
		var l_act=l___this__1[|l_i];
		if(l_i>0)l_r+="\n";
...
var __this__=[mt_gml_func_scope];
array_copy(__this__,1,mq_gml_func_scope,1,9);
if (live_enabled) {
	__this__[@8]=undefined;
	__this__[@7]=undefined;
	__this__[@5]=array_create(16,0);
	__this__[@1]=argument[0];
	__this__[@2]=argument[1];
	__this__[@3]=argument[2];
	__this__[@4]=argument[3];
	__this__[@6]=[2,argument[5],argument[4]];
	__this__[@9]=argument[6];
}
return __this__;
...
if (live_enabled) {
	...
	__this__[@6]=undefined;
	__this__[@3]=undefined;
	__this__[@2]=gml_thread_status_none;
	__this__[@1]=undefined;
	__this__[@0]=argument[0];
	if(l_self1==undefined)l_self1=self.id;
	if(l_other1==undefined)l_other1=other.id;
	__this__[@4]=gml_func_scope_create(argument[1],l_offset,argument[2],argument[3],l_self1,l_other1,undefined);
}
return __this__;
...
var __this__=argument[0];
if (live_enabled) {
	if(__this__[8]!=undefined)return gml_pos_to_string(__this__[8]); else return "?";
} else return undefined;

#define gml_thread_proc_error
var __this__=argument[0],l_act=argument[2];
if (live_enabled) {
	var l_pos=l_act[1];
	__this__[@7]=gml_pos_to_string(l_pos)+" "+argument[1];
	__this__[@8]=l_pos;
	__this__[@2]=gml_thread_status_error;
	return false;
} else return false;
...
if (live_enabled) {
	...
	l_stack=l_scope[5];
	var l__callback;
	__this__[@2]=gml_thread_status_running;
	while(true){
		...
		while(l_proc&&l_pos<l_len){
			...
			switch(l_act[0]){
				...
				case 67:tools__gml_stack_gml_stack_impl__push(l_stack,__this__[1]);break;
				case 7:
					l_n=l_act[2];
				...
				case 44:
					...
					if(gml_value_is_number(l_v1)){
						if(gml_value_is_number(l_v2))l_v1=l_v1+l_v2; else l_proc=gml_thread_proc_error(l__g__this__,"Can't add number and "+gml_value_get_type(l_v2),l_act);
					} else if(is_string(l_v1)){
						if(is_string(l_v2))l_v1=l_v1+l_v2; else l_proc=gml_thread_proc_error(l__g__this__,"Can't add string and "+gml_value_get_type(l_v2),l_act);
					} else l_proc=gml_thread_proc_error(l__g__this__,"Can't add "+gml_value_get_type(l_v1)+" and "+gml_value_get_type(l_v2),l_act);
				...
				case 46:
					...
					l_op=l_act[2];
					if(gml_value_is_number(l_v1)){
						if(gml_value_is_number(l_v2))l_v1=gml_op_apply(l_op,l_v1,l_v2); else l_proc=gml_thread_proc_error(l__g__this__,"Can't apply "+gml_op_to_string(l_op)+" to number and "+gml_value_get_type(l_v2),l_act);
					} else l_proc=gml_thread_proc_error(l__g__this__,"Can't apply "+gml_op_to_string(l_op)+" to "+gml_value_get_type(l_v1)+" and "+gml_value_get_type(l_v2),l_act);
					tools__gml_stack_gml_stack_impl__push(l_stack,l_v1);
					break;
				case 47:
					var l__o7=l_act[2];
					...
					} else l_proc=gml_thread_proc_error(l__g__this__,"Value must be a number.",l_act);
					break;
				case 42:
					l_v1=tools__gml_stack_gml_stack_impl__pop(l_stack);
					l_v2=tools__gml_stack_gml_stack_impl__pop(l_stack);
				...
				case 41:
					var l_v9=l_act[2];
					l_v1=tools__gml_stack_gml_stack_impl__pop(l_stack);
					if(gml_value_is_number(l_v1))tools__gml_stack_gml_stack_impl__push(l_stack,l_v1+l_v9); else l_proc=gml_thread_proc_error(l__g__this__,"Can't add "+gml_value_get_type(l_v1)+" and "+string(l_v9),l_act);
					break;
				...
				case 15:
					l_s=l_act[2];
					if(variable_global_exists(l_s))tools__gml_stack_gml_stack_impl__push(l_stack,variable_global_get(l_s)); else l_proc=gml_thread_proc_error(l__g__this__,"Global variable `"+l_s+"` is not set.",l_act);
					break;
				case 16:
					l_s=l_act[2];
					l_v1=tools__gml_stack_gml_stack_impl__pop(l_stack);
				...
				case 17:
					...
					if(variable_global_exists(l_s)){
						...
						variable_global_set(l_s,l_v1);
					} else l_proc=gml_thread_proc_error(l__g__this__,"Global variable `"+l_s+"` is not set.",l_act);
					break;
				case 18:
					...
					if(__this__[2]==gml_thread_status_error)l_proc=gml_thread_proc_error(l__g__this__,__this__[7],l_act);
					break;
				case 19:
					l_v1=tools__gml_stack_gml_stack_impl__pop(l_stack);
				...
				case 20:
					...
					if(__this__[2]!=gml_thread_status_error){
						if(l_op==16){
							if(gml_value_is_number(l_v1)){
								if(gml_value_is_number(l_v2))l_v1=l_v1+l_v2; else l_proc=gml_thread_proc_error(l__g__this__,"Can't add number and "+gml_value_get_type(l_v2),l_act);
							} else if(is_string(l_v1)){
						...
						if(__this__[2]==gml_thread_status_error)l_proc=gml_thread_proc_error(l__g__this__,__this__[7],l_act);
					} else l_proc=gml_thread_proc_error(l__g__this__,__this__[7],l_act);
					break;
				case 48:
					var l__not=l_act[2];
					l_v2=tools__gml_stack_gml_stack_impl__pop(l_stack);
					...
					if(is_string(l_v1)){
						...
						l_i=(l_z!=l__not?1:0);
						tools__gml_stack_gml_stack_impl__push(l_stack,l_i);
					} else l_proc=gml_thread_proc_error(l__g__this__,"Field name `"+gml_value_print(l_v1)+"` should be a string",l_act);
					break;
				case 73:
					l_v2=tools__gml_stack_gml_stack_impl__pop(l_stack);
					l_v1=tools__gml_stack_gml_stack_impl__pop(l_stack);
					if(gml_value_is_number(l_v2)){
						...
						} else l_proc=gml_thread_proc_error(l__g__this__,"Alarm index ("+gml_value_print(l_v2)+") shold be between 0 and 11",l_act);
					} else l_proc=gml_thread_proc_error(l__g__this__,"Alarm index ("+gml_value_print(l_v2)+") shold be a number",l_act);
					break;
				case 74:
					l_v3=tools__gml_stack_gml_stack_impl__pop(l_stack);
					l_v2=tools__gml_stack_gml_stack_impl__pop(l_stack);
					...
					if(gml_value_is_number(l_v2)){
						...
						if(l_i>=0&&l_i<12){
							l_z=true;
							with (l_v1) {
								if(gml_value_is_number(l_v3))self.alarm[l_i] = round(l_v3); else l_proc=gml_thread_proc_error(l__g__this__,"Alarm value ("+gml_value_print(l_v3)+") should be a number",l_act);
								l_z=false;
							}
							if(l_z){
								if(object_exists((l_v1|0)))gml_thread_error("Couldn't find any instances of "+gml_std_Std_string(l_v1)+" ("+object_get_name((l_v1|0))+")"); else gml_thread_error("Couldn't find instance `"+gml_std_Std_string(l_v1)+"`");
					...
					if(gml_value_is_number(l_v4)){
						...
						if(l_i>=0&&l_i<12){
							...
							with (l_v1) {
								...
								} else l_proc=gml_thread_proc_error(l__g__this__,"Alarm value ("+gml_value_print(l_v2)+") should be a number",l_act);
								l_z=false;
							}
							if(l_z){
								if(object_exists((l_v1|0)))gml_thread_error("Couldn't find any instances of "+gml_std_Std_string(l_v1)+" ("+object_get_name((l_v1|0))+")"); else gml_thread_error("Couldn't find instance `"+gml_std_Std_string(l_v1)+"`");
							}
						} else l_proc=gml_thread_proc_error(l__g__this__,"Alarm index ("+gml_value_print(l_v4)+") shold be between 0 and 11",l_act);
					} else l_proc=gml_thread_proc_error(l__g__this__,"Alarm index ("+gml_value_print(l_v4)+") shold be a number",l_act);
					break;
				case 70:
					l_i=l_act[2];
				...
				case 72:
					...
					if(__this__[2]==gml_thread_status_error){
						l_proc=gml_thread_proc_error(l__g__this__,__this__[7],l_act);
				...
				case 21:
					...
					if(is_array(l_v1)){
						if(gml_value_is_number(l_v2)){
							...
							if(l_i>=0&&l_i<l_n)tools__gml_stack_gml_stack_impl__push(l_stack,l_w1[l_i]); else l_proc=gml_thread_proc_error(l__g__this__,"Index ("+string(l_i)+") is out of bounds (0.."+string(l_n)+" excl.)",l_act);
						} else l_proc=gml_thread_proc_error(l__g__this__,"Index is not a number",l_act);
					} else l_proc=gml_thread_proc_error(l__g__this__,"Variable is not an array",l_act);
					break;
				case 22:
					l_v3=tools__gml_stack_gml_stack_impl__pop(l_stack);
					l_v2=tools__gml_stack_gml_stack_impl__pop(l_stack);
					...
					if(is_array(l_v1)){
						if(gml_value_is_number(l_v2)){
							...
							l_w1=l_v1;
							if(l_i<0){
								l_proc=gml_thread_proc_error(l__g__this__,"Index ("+string(l_i)+") may not be negative",l_act);
							} else if(l_i>=32000){
								l_proc=gml_thread_proc_error(l__g__this__,"Index ("+string(l_i)+") may not exceed 32000",l_act);
							} else l_w1[@l_i]=l_v3;
						} else l_proc=gml_thread_proc_error(l__g__this__,"Index is not a number",l_act);
					} else l_proc=gml_thread_proc_error(l__g__this__,"Variable is not an array",l_act);
					break;
					...
					if(is_array(l_v1)){
						if(gml_value_is_number(l_v3)){
							...
							if(l_i<0){
								...
								l_proc=gml_thread_proc_error(l__g__this__,"Index ("+string(l_i)+") is out of bounds (0.."+string(l_n)+" excl.)",l_act);
							} else {
								l_v1=l_w1[l_i];
								l_op=l_o2;
								if(l_op==16){
				...
				case 24:
					...
					if(is_array(l_v1)){
						if(gml_value_is_number(l_v2)){
							...
							if(l_i<0||l_i>=l_n){
								l_proc=gml_thread_proc_error(l__g__this__,"Row index ("+string(l_i)+") is out of bounds (0.."+string(l_n)+" excl.)",l_act);
							} else if(gml_value_is_number(l_v3)){
								l_k=(l_v3|0);
								l_n=array_length_2d(l_v1,l_i);
								if(l_k<0||l_k>=l_n)l_proc=gml_thread_proc_error(l__g__this__,"Column index ("+string(l_k)+") is out of bounds (0.."+string(l_n)+" excl.)",l_act); else tools__gml_stack_gml_stack_impl__push(l_stack,l_v1[l_i, l_k]);
							} else l_proc=gml_thread_proc_error(l__g__this__,"Column index is not a number",l_act);
						} else l_proc=gml_thread_proc_error(l__g__this__,"Row index is not a number",l_act);
					} else l_proc=gml_thread_proc_error(l__g__this__,"Variable is not an array",l_act);
					break;
				case 25:
					l_v4=tools__gml_stack_gml_stack_impl__pop(l_stack);
					...
					if(is_array(l_v1)){
						if(gml_value_is_number(l_v2)){
							if(gml_value_is_number(l_v3)){
								...
								l_k=(l_v3|0);
								if(l_i<0){
									l_proc=gml_thread_proc_error(l__g__this__,"Row index ("+string(l_i)+") may not be negative",l_act);
								} else if(l_i>=32000){
									l_proc=gml_thread_proc_error(l__g__this__,"Row index ("+string(l_i)+") may not exceed 32000",l_act);
								} else if(l_k<0){
									l_proc=gml_thread_proc_error(l__g__this__,"Column index ("+string(l_k)+") may not be negative",l_act);
								} else if(l_k>=32000){
									l_proc=gml_thread_proc_error(l__g__this__,"Column index ("+string(l_k)+") may not exceed 32000",l_act);
						...
						if(gml_value_is_number(l_v2)){
							if(gml_value_is_number(l_v3)){
								...
								if(l_i<0||l_i>=array_height_2d(l_w1)){
									...
									l_proc=gml_thread_proc_error(l__g__this__,"Column index ("+string(l_k)+") is out of bounds (0.."+string(l_n)+" excl.)",l_act);
								} else {
									l_v1=l_w1[l_i, l_k];
									l_v2=l_v4;
									l_op=l__op;
				...
				case 28:
					...
					if(l_s==""){
						...
						if(__this__[2]==gml_thread_status_error)l_proc=gml_thread_proc_error(l__g__this__,__this__[7],l_act);
					} else l_proc=gml_thread_proc_error(l__g__this__,"Type check failure for value ("+gml_value_print(l_v1)+"): "+l_s,l_act);
					break;
				case 29:
					var l__func=l_act[3];
					var l__o2=l_act[2];
				...
				case 30:
					...
					if(gml_value_is_number(l_v3)){
						...
						if(__this__[2]==gml_thread_status_error)l_proc=gml_thread_proc_error(l__g__this__,__this__[7],l_act);
					} else l_proc=gml_thread_proc_error(l__g__this__,"Index "+gml_value_print(l_v3)+" is not a number.",l_act);
					break;
				case 31:
					var l_ch1=l_act[3];
					var l_f6=l_act[2];
					...
					} else l_proc=gml_thread_proc_error(l__g__this__,"Type check failure for value ("+gml_value_print(l_v2)+"): "+l_s,l_act);
					break;
				case 32:
				...
				case 34:
					l_i=l_act[2];
					l_n=array_length_1d(l_args1);
					if(l_i>=0&&l_i<l_n)tools__gml_stack_gml_stack_impl__push(l_stack,l_args1[l_i]); else l_proc=gml_thread_proc_error(l__g__this__,"Argument index "+string(l_i)+" is out of range (0.."+string(l_n)+" excl)",l_act);
					break;
				case 35:
					l_i=l_act[2];
					l_n=array_length_1d(l_args1);
					if(l_i>=0&&l_i<l_n)l_args1[@l_i]=tools__gml_stack_gml_stack_impl__pop(l_stack); else l_proc=gml_thread_proc_error(l__g__this__,"Argument index "+string(l_i)+" is out of range (0.."+string(l_n)+" excl)",l_act);
					break;
				case 36:
					var l__i3=l_act[3];
					var l__o5=l_act[2];
					...
					if(l_i>=0&&l_i<l_n){
						...
						l_args1[@l_i]=l_v1;
					} else l_proc=gml_thread_proc_error(l__g__this__,"Argument index "+string(l_i)+" is out of range (0.."+string(l_n)+" excl)",l_act);
					break;
				case 37:
					l_v1=tools__gml_stack_gml_stack_impl__pop(l_stack);
					if(gml_value_is_number(l_v1)){
				...
				case 38:
					...
					if(gml_value_is_number(l_v1)){
						l_i=(l_v1|0);
						l_n=array_length_1d(l_args1);
						if(l_i>=0&&l_i<l_n)l_args1[@l_i]=l_v2; else l_proc=gml_thread_proc_error(l__g__this__,"Argument index "+string(l_i)+" is out of range (0.."+string(l_n)+" excl)",l_act);
					} else l_proc=gml_thread_proc_error(l__g__this__,"Index is not a number",l_act);
					break;
				case 39:
					var l__o6=l_act[2];
				...
				case 50:
					...
					if(gml_value_is_number(l_v1)){
						...
						var l_tmp;
						if(l_k>=0)l_tmp=l_k<array_length_1d(gml_std_haxe_boot_wget(__this__[0],2)); else l_tmp=false;
						if(l_tmp){
							l_scr=gml_std_haxe_boot_wget(gml_std_haxe_boot_wget(__this__[0],2),l_k);
							l_scope[@2]=l_pos;
							l_scope[@6]=l_inst;
							l_scope[@7]=l_with1;
							var l_scr2=l_scr[10];
							...
							l_len=ds_list_size(l_actions);
							l_stack=l_scope[5];
						} else l_proc=gml_thread_proc_error(l__g__this__,("Script index `"+gml_value_print(l_v1)+"` is out of bounds ("+string(g_gml_script_index_offset)+"..")+string(g_gml_script_index_offset+array_length_1d(gml_std_haxe_boot_wget(__this__[0],2)))+" excl.)",l_act);
					} else {
						tools__gml_stack_gml_stack_impl__discard_multi(l_stack,l_n+1);
						l_proc=gml_thread_proc_error(l__g__this__,"Script index must be a number, got "+gml_value_dump(l_v1),l_act);
					}
					break;
				case 51:
				...
				case 52:
					...
					if(l_i<l_n){
						l_proc=gml_thread_proc_error(l__g__this__,"Type check failure for argument["+string(l_i)+"] ("+gml_value_print(l_w1[l_i])+"): "+l_s,l_act);
					} else {
						__this__[@8]=l_act[1];
						l_i=l_act[6];
						switch(l_i){
							case 3:
								...
								if(l_qn0==l_qc0&&l_qn1==l_qc1){
									...
									if(l_z){
										...
										if(l_z)l_proc=gml_thread_proc_error(l__g__this__,"Can't call instance-specific function - instance `"+gml_value_print(l_inst[l_inst[0]])+"` does not exist.",l_act);
									}
								}
								break;
							case 2:
				...
				case 60:
					...
					} else l_proc=gml_thread_proc_error(l__g__this__,"Repeat `times` must be a number.",l_act);
					break;
				case 59:
					var l_p6=l_act[2];
				...
				case 68:
					l_v1=tools__gml_stack_gml_stack_impl__pop(l_stack);
					__this__[@2]=gml_thread_status_waiting;
					__this__[@5]=(gml_value_is_number(l_v1)?l_v1:0);
					l_scope[@2]=l_pos;
					l_scope[@6]=l_inst;
					l_scope[@7]=l_with1;
					ds_list_add(gml_std_haxe_boot_wget(__this__[0],7),__this__);
					l_proc=false;
					break;
				case 69:
					l_z=l_act[2];
					...
					l_scope[@7]=l_with1;
					__this__[@4]=l_scope;
					gml_thread_exec(gml_thread_fork(__this__));
		...
		if(__this__[2]==gml_thread_status_error){
			...
			if(l_scope==undefined){
				...
				__this__[@1]=undefined;
				l__callback=l__g__this__[3];
				if(l__callback!=undefined)script_execute(l__callback,l__g__this__);
				break;
			} else if(l_proc){
				if(l_with1!=undefined)gml_with_scope_destroy(l_with1);
				l_scope[@2]=l_pos;
		...
		} else if(l_pos>=l_len&&__this__[2]==gml_thread_status_running){
			if(l_with1!=undefined)gml_with_scope_destroy(l_with1);
			if(l_stack[0]>0)__this__[@1]=l_stack[l_stack[0]]; else __this__[@1]=0;
			l_yycNext=l_scope[9];
			l_scope=l_yycNext;
			if(l_scope!=undefined){
				l_args1=l_scope[3];
				...
				__this__[@2]=gml_thread_status_done;
				l__callback=l__g__this__[3];
				if(l__callback!=undefined)script_execute(l__callback,l__g__this__);
				break;
			}
...
if (live_enabled) {
	var l_q=__this__[4];
	var l_q_inst=l_q[6];
	var l_r=gml_thread_create(__this__[0],l_q[1],gml_value_list_copy(l_q[3]),gml_value_list_copy(l_q[4]),l_q_inst[l_q_inst[0]],l_q_inst[l_q_inst[0]-1],l_q[2]);
	l_r[@3]=__this__[3];
	var l_n=l_r[4];
...
if (live_enabled) {
	gml_thread_error("Calls only support up to "+string(81)+" arguments at __this__ time");
	return undefined;
} else return undefined;
...
var l___this__1=argument[0];
if (live_enabled) {
	return (is_real(l___this__1)||is_int64(l___this__1)||is_bool(l___this__1))||is_int32(l___this__1);
} else return false;

#define gml_value_get_type
var l___this__1=argument[0];
if (live_enabled) {
	if(l___this__1==undefined){
		return "undefined";
	} else if(gml_value_is_number(l___this__1)){
		return "number";
	} else if(is_string(l___this__1)){
		return "string";
	} else if(is_array(l___this__1)){
		if(array_length_2d(l___this__1,2)==1){
			return "lwobject";
		} else if(array_height_2d(l___this__1)==2){
			var l___this__2=l___this__1;
			return script_execute(l___this__2[1, 0],5,l___this__2);
		} else if(is_array(l___this__1)){
			return "array";
		} else return "ref";
	} else return typeof(l___this__1);
} else return undefined;
	...
	if(l_v==undefined){
		...
		if(array_length_2d(l_v,2)==1){
			...
			if(is_array(l_v[1, 0])){
				...
				var l___this__1=l_v;
				return script_execute(l___this__1[1, 0],6,l___this__1);
			}
		} else {
...
var l___this__1=argument[0];
if (live_enabled) {
	return "`"+gml_value_print(l___this__1)+"` ("+gml_value_get_type(l___this__1)+")";
} else return undefined;

#define gml_value_list_copy
var l___this__1=argument[0];
if (live_enabled) {
	var l_n=array_length_1d(l___this__1);
	var l_r=array_create(l_n);
	array_copy(l_r,0,l___this__1,0,l_n);
	return l_r;
} else return undefined;
...
var __this__=[mt_gml_with_scope];
array_copy(__this__,1,mq_gml_with_scope,1,5);
...
var l___this__1=argument[0];
if (live_enabled) {
	if(--l___this__1[2]!=0)return 0;
	var l_size=l___this__1[0];
	var l_sln=(l_size>0?(log10(l_size)|0):0);
	if(l_sln>=5)return 0;
	ds_stack_push(g_vm__gml_with_data_gml_with_data_impl__pools[l_sln],l___this__1[1]);
}
