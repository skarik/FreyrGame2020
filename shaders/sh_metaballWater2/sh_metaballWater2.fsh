...
void main()
{
	...
	vec4 l_peturbation = vec4(0.0, 0.0, 0.0, 0.0);//texture2D( samplerPeturb, fract(l_worldPosition.xy / 32.0) );
	...
	// Check if on edge
	float l_pixelMetaballEdge1 = 1.0; //texture2D( gm_BaseTexture, v_vTexcoord + l_edgePush1 * vec2(uWaterTexelSize.x, 0.0) ).a;
	float l_pixelMetaballEdge2 = 1.0; //texture2D( gm_BaseTexture, v_vTexcoord + l_edgePush2 * vec2(0.0, uWaterTexelSize.y) ).a;
	float l_pixelMetaballEdge3 = 1.0; //texture2D( gm_BaseTexture, v_vTexcoord - l_edgePush3 * vec2(uWaterTexelSize.x, 0.0) ).a;
	float l_pixelMetaballEdge4 = 1.0; //texture2D( gm_BaseTexture, v_vTexcoord - l_edgePush4 * vec2(0.0, uWaterTexelSize.y) ).a;
