...
varying vec2 v_vScreenPosition;

uniform vec4 uTexSheetCoords;

uniform sampler2D samplerArtMesh;
...
void main()
{
	...
	//gl_FragColor.a = 1.0;
	gl_FragColor = texture2D( samplerArtMesh, v_vScreenPosition );
	//gl_FragColor.rg = v_vScreenPosition;
}
