...
function cutsceneLoad(argument0) {
	cutsceneInit();
	    ...
	    if (read_state == STATE_READ_OBJECT)
	    {
	        ...
	        if (string_pos("}", line) != 0)
	        {
					...
					|| read_object_type == SEQTYPE_GOTO_IF_TIME)
				{
					...
					{
						var t_try, with_next = undefined, notwith_next = undefined, lookup;
				...
				else if (read_object_type == SEQTYPE_GIVEITEM)
				{
					...
					{
						var t_try, item_next = undefined, lookup;
						t_try = 0;
						lookup = "item";
						do
						{
	        ...
	        if (!ds_map_add(read_object_map, key, value))
	        {
				...
				var t_try = 0;
				while (!ds_map_add(read_object_map, key + string(t_try), value))
				{
					t_try += 1;
				}
				debugOut("Renamed as \"" + key + string(t_try) + "\" and added.");
	        }
	    }
	}
