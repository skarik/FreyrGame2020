function _playerMoAttack1() {
	// Increment timer
	var l_meleeAtkTimerPrev = meleeAtkTimer;
	meleeAtkTimer += Time.deltaTime;
	// Get percent of time
	var l_meleePercent = clamp(meleeAtkTimer / meleeAtk1Time, 0.0, 1.0);
	...
	var kMeleeAtkEarlyKey = lerp(meleeAtk0Hit, meleeAtk1Key, 0.5);
	if (meleeAtkTimer < meleeAtk1Hit || meleeAtkTimer > kMeleeAtkEarlyKey)
	{
		if (meleeDashQueued)
		{
			isAttacking = false;
	...
	if (meleeAtkTimer > meleeAtk1Time)
	{
		meleeDashQueued = false;
		meleeAtkQueued = false;
		meleeAtkTimer = 0.0;
